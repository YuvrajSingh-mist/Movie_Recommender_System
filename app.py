...
movies_df = pd.read_csv('movies_refined_dataset.csv')
...
def recommend_movie(index):
    ...
    for i in movie_indexes:
        ...
        movie_names.append(movies_df.iloc[i[0], 2])
        # print(movie)
    return movie_names
...
movie_names_list = list(movies_df.iloc[:, 2])

name = st.selectbox("Choose the movie for which you want recommendations from below", options=movie_names_list)

print(movies_df)

#Button to how recommendations

if st.button('Show Top 5 Similar Movies'):
